...
export interface TripPayload {
  ...
  recetteChauffeurInitial?: number | null;
  recetteChauffeurRemplacant?: number | null;
  prefinancementChauffeurInitial?: number | null;
  prefinancementChauffeurRemplacant?: number | null;
  dateDepart: string;
  dateArrivee?: string;
  recette: number;
...
export const personnelApi = {
  getAll: () => request<any[]>('/personnel'),
  getOne: (id: string) => request<any>(`/personnel/${id}`),
  create: (data: PersonnelPayload) => request<any>('/personnel', { method: 'POST', body: JSON.stringify(data) }),
  update: (id: string, data: Partial<PersonnelPayload>) => request<any>(`/personnel/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
