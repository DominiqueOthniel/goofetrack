...
const navigation = [
  ...
  { name: 'Personnel',  href: '/personnel',  icon: BriefcaseBusiness, color: 'from-indigo-500 to-sky-500' },
...
export const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="min-h-screen bg-background relative">
      {/* Alertes système */}
      {apiError && (
        <Alert className="rounded-none border-x-0 border-t-0 z-50 relative bg-amber-50 text-amber-900 border-amber-200 dark:bg-amber-950/30 dark:text-amber-200 dark:border-amber-900">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            Connexion aux données momentanément indisponible. Vous pouvez continuer à consulter l’interface ; si besoin, démarrez le backend avec <span className="font-mono">cd backend && npm run start:dev</span>.
          </AlertDescription>
        </Alert>
      )}
      {isLoading && (
